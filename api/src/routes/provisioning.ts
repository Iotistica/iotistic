...
async function createVpnCredentials(uuid: string, vpnConfig: any): Promise<{ username: string; password: string; caCert: string }> {
  ...
  logger.info(` Fetching VPN CA certificate from: ${vpnCaUrl}`);
  ...
  try {
    ...
    if (!caCert) {
      ...
      logger.info(` VPN CA certificate fetched (${caCert.length} bytes)`);
    } else {
      logger.info(` Using cached VPN CA certificate (${caCert.length} bytes)`);
    }
    ...
    logger.info(` VPN credentials created for device: ${vpnUsername}`);
    return { username: vpnUsername, password: vpnPassword, caCert };
  } catch (error: any) {
    logger.error(` Failed to create VPN credentials:`, error.message);
...
async function buildProvisioningResponse(
  ...
  if (vpnEnabled && vpnCredentials) {
    ...
    logger.info(` VPN configuration added to provisioning response`);
  }
  ...
  try {
    ...
    if (preRegisteredDevice) {
      ...
      if (vpnConfigPreReg && vpnConfigPreReg.enabled) {
        try {
          ...
          logger.info(` VPN credentials created for pre-registered device`);
        } catch (error: any) {
    ...
    if (vpnConfig && vpnConfig.enabled) {
      try {
        ...
        logger.info(` VPN credentials created for device: ${uuid.substring(0, 8)}...`);
      } catch (error: any) {
        logger.error(`  VPN credential creation failed:`, error.message);
