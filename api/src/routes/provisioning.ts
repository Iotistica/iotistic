...
router.post('/provisioning-keys/generate', async (req, res) => {
  try {
    ...
    if (newKey && previousKeyId) {
      try {
        console.log(`🔄 Invalidating previous provisioning key: ${previousKeyId}`);
        await revokeProvisioningKey(previousKeyId, 'Regenerated by user');
      } catch (revokeError: any) {
        console.warn(`⚠️ Could not revoke previous key ${previousKeyId}:`, revokeError.message);
    ...
    console.log(`✅ Single-device provisioning key generated: ${id}`);
